...
//  COMPLETED (20) Create a constant int to identify our loader used in DetailActivity
    private static final int ID_LOADER = 200;
...
//      COMPLETED (13) Find each of the TextViews by ID
        ...
        mHighTempTextView = (TextView) findViewById(R.id.high_temp);
        mLowTempTextView = (TextView) findViewById(R.id.low_temp);
...
//      COMPLETED (35) Initialize the loader for DetailActivity
        getSupportLoaderManager().initLoader(ID_LOADER, null, null);
    }
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        if (data != null && data.moveToFirst()){
            ...
            String humidityString = "Humidity: " +Float.toString(humidity);
            mHumidityTextView.setText(humidityString);

            float pressure = data.getFloat(INDEX_WEATHER_PRESSURE);
            String pressureString = "Pressure: " +Float.toString(pressure);
            mPressureTextView.setText(pressureString);

            float wind = data.getFloat(INDEX_WEATHER_WIND);
            float direction = data.getFloat(INDEX_WEATHER_DIRECTION);
            String windString = "Wind: " + Float.toString(wind) + " - Direction: "+ Float.toString(direction);
            mWindTextView.setText(windString);

            String summary = dateString + ": " + description;
